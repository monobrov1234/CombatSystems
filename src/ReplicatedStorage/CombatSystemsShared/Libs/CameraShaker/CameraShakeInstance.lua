...
function CameraShakeInstance:UpdateShake(dt)
	...
	if not self.sustain then currentFadeTime = currentFadeTime - (dt / self.fadeOutDuration) end
...
function CameraShakeInstance:StartFadeOut(fadeOutTime)
	if fadeOutTime == 0 then self.currentFadeTime = 0 end
...
function CameraShakeInstance:StartFadeIn(fadeInTime)
	if fadeInTime == 0 then self.currentFadeTime = 1 end
