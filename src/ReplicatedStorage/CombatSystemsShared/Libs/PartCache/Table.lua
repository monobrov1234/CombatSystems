...
Table.keyOf = function(tbl, value)
	for index, obj in pairs(tbl) do
		if obj == value then return index end
	end
	return nil
end
...
Table.removeObject = function(tbl, obj)
	...
	if index then table.remove(tbl, index) end
end
...
Table.expand = function(tbl, byAmount)
	if byAmount < 0 then error("Cannot expand a table by a negative amount of objects.") end
